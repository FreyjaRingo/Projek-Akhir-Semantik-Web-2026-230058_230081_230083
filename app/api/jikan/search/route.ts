...
const searchSchema = z.object({
  ...
  order_by: z.enum(['score', 'popularity', 'rank', 'members', 'title', 'start_date', 'end_date']).default('score'),
...
// GET /api/jikan/search - Search anime from Jikan API
export async function GET(request: NextRequest) {
  ...
  const { q, page, limit, type, status, order_by, sort, genres, min_score, max_score, start_date, end_date } = parsed.data;
  ...
  try {
    ...
    const response = await fetch(`${JIKAN_BASE_URL}/anime?${jikanParams}`);
    ...
    console.error('GET /api/jikan/search error:', error);
    return NextResponse.json(
